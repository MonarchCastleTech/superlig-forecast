...
The interface uses a modern sports-broadcast and analytics-lab direction rather
than a generic administration dashboard:

- a dark stadium-inspired canvas with the existing lime and coral probability
  accents;
- scoreboard-like run counters, elapsed time, simulation rate, and progress;
- crisp condensed headings, tabular numeric typography, and compact club rows;
- animated live-state indicators and restrained probability-line motion;
- dense but legible desktop analysis views with stacked mobile controls;
- no decorative imagery that competes with the forecast data.

The official Monarch Castle Technologies logo appears in the masthead and
footer with a short `Forecasting Intelligence` product descriptor. The logo:

- is sourced from an official MCT-controlled public repository or brand asset;
- is copied into the repository so GitHub Pages does not hot-link it;
- preserves its original aspect ratio, colors, and clear space;
- includes accessible alternative text and an organization link;
- is never redrawn, approximated, or generated by an image model.
...
- official local MCT logo is rendered in the masthead and footer;
- sports-style live scoreboard remains usable at desktop and mobile widths;
...
10. The official MCT logo and sports-broadcast visual system are present without
    weakening chart legibility or accessibility.
11. No repository or Pages deployment is created until the user names and
    authorizes a target.

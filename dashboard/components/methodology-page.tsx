...
export function MethodologyPage({ data }: { data: DashboardPayload }) {
  ...
  const repository = "https://github.com/MonarchCastleTech/superlig-forecast/blob/main";
  return (
    <main className="methodology-page">
      ...
      <article className="methodology-paper">
        <nav aria-label="Methodology contents">
          <a href="#production">0. Used now</a><a href="#target">1. Target</a><a href="#sources">2. Sources</a><a href="#model">3. Model</a>
          <a href="#simulation">4. Simulation</a><a href="#validation">5. Validation</a><a href="#reproduce">6. Reproduce</a>
        </nav>

        <section id="production" className="production-disclosure">
          <span>00</span><div><h2>What the homepage uses now</h2>
          <p><strong>The published homepage is generated by the method described on this page.</strong> The methodology page is documentation; it is not a second model and the homepage does not “call” this page. Both pages read the same published <code>dashboard.json</code> record, which is produced by the Python <code>forecast-season</code> command. The text below describes that current executable path—not a planned or alternative model.</p>
          <ol className="runtime-chain" aria-label="Production calculation path">
            <li><strong>Observed state:</strong> accepted TFF results fix the points and goals already earned.</li>
            <li><strong>Fixture probabilities:</strong> the frozen historical team-strength artifact and current aggregate Transfermarkt squad values produce a score matrix for every unplayed home/away pairing.</li>
            <li><strong>Season probabilities:</strong> the simulator samples {formatInteger(data.meta.simulations)} complete remaining-season paths with seed {data.meta.seed} and counts first-place finishes.</li>
            <li><strong>Publication:</strong> the resulting <code>dashboard.json</code> supplies the probabilities shown on the homepage and the run metadata shown here.</li>
          </ol>
          <table>
            <caption>Production inclusion audit</caption>
            <thead><tr><th>Layer</th><th>Used in title probabilities?</th><th>Current implementation</th></tr></thead>
            <tbody>
              <tr><td data-label="Layer">Historical team strength</td><td data-label="Used?"><strong>Yes</strong></td><td data-label="Implementation">Five-season, recency-weighted home/away scoring and conceding ratios with six-match shrinkage.</td></tr>
              <tr><td data-label="Layer">Current results</td><td data-label="Used?"><strong>Yes</strong></td><td data-label="Implementation">Completed TFF scores set points, goals for, and goals against before simulation.</td></tr>
              <tr><td data-label="Layer">Squad information</td><td data-label="Used?"><strong>Yes, aggregate only</strong></td><td data-label="Implementation">Transfermarkt club-level squad value changes expected-goal balance with fixed coefficient {data.meta.value_coefficient.toFixed(2)}.</td></tr>
              <tr><td data-label="Layer">Match outcomes</td><td data-label="Used?"><strong>Yes</strong></td><td data-label="Implementation">Independent Poisson scores plus fixed Dixon–Coles low-score correction ρ = −0.05.</td></tr>
              <tr><td data-label="Layer">Season outcomes</td><td data-label="Used?"><strong>Yes</strong></td><td data-label="Implementation">{formatInteger(data.meta.simulations)} seeded Monte Carlo paths.</td></tr>
              <tr><td data-label="Layer">Individual minutes, injuries, lineups, xG, odds</td><td data-label="Used?"><strong>No</strong></td><td data-label="Implementation">Not consumed by the production title forecast. Odds occur only in backtest comparison baselines.</td></tr>
            </tbody>
          </table>
          <p className="implementation-links">Executable evidence: <a href={`${repository}/src/superlig_forecast/forecasting/current_season.py`}>fixture construction</a>, <a href={`${repository}/src/superlig_forecast/modeling/team_strength.py`}>team-strength estimator</a>, <a href={`${repository}/src/superlig_forecast/simulation/season.py`}>season simulator</a>, and <a href={`${repository}/.github/workflows/update-forecast.yml`}>six-hour publication workflow</a>.</p>
          <aside><strong>Current published-run audit.</strong> Generated: {formatForecastUpdate(data.freshness.generated_at)}. Match snapshot: {formatForecastUpdate(data.freshness.match_snapshot_at)}. Squad/value snapshot: {formatForecastUpdate(data.freshness.valuation_snapshot_at)}. Completed fixtures: {data.meta.completed_fixture_count}/{data.meta.fixture_count}. Source match: {source ? `${source.matched_team_count}/${source.official_team_count}` : "unavailable"}. Model: {data.meta.model_version}; value coefficient: {data.meta.value_coefficient.toFixed(2)}; paths: {formatInteger(data.meta.simulations)}; seed: {data.meta.seed}. Every value is read from the same payload displayed by the homepage.</aside></div>
        </section>
